;;; this code is simply go-mode.el that has been
;;; find replaced with s/go/ally with slight modification
;;; for indentation.

;;; ally-mode.el --- Major mode for the Ally programming language
;;; Commentary:

;; For installation instructions, see ally-mode-load.el

;;; To do:

;; * Indentation is *almost* identical to allyfmt
...
;;    right
;; * Format region using allyfmt

;;; Code:

(eval-when-compile (require 'cl))

(defvar ally-mode-syntax-table
  (let ((st (make-syntax-table)))
    ...
    st)
  "Syntax table for Ally mode.")

(defvar ally-mode-keywords
  '("break"    "default"     "def"   "interface" "select" "module"
    "case"     "defer"       "ally"     "map"       "struct"
    "chan"     "else"        "goto"   "package"   "switch"
    "const"    "fallthrough" "if"     "range"     "type"
    "continue" "for"         "import" "return"    "var")
  "All keywords in the Ally language.  Used for font locking and
some syntax analysis.")

(defvar ally-mode-font-lock-keywords
  (let ((builtins '("append" "cap" "close" "complex" "copy" "imag" "len"
                    "make" "new" "panic" "print" "println" "real" "recover"))
        (constants '("nil" "true" "false" "iota"))
        (type-name "\\s *\\(?:[*(]\\s *\\)*\\(?:\\w+\\s *\\.\\s *\\)?\\(\\w+\\)")
        )
    `((,(regexp-opt ally-mode-keywords 'words) . font-lock-keyword-face)
...
;;      (,(concat "]" type-name) 1 font-lock-type-face)
  ...
  "Basic font lock keywords for Ally mode.  Highlights keywords,
...
(defvar ally-mode-map
  (let ((m (make-sparse-keymap)))
    (define-key m "}" #'ally-mode-insert-and-indent)
    (define-key m ")" #'ally-mode-insert-and-indent)
    (define-key m ":" #'ally-mode-delayed-electric)
    ;; In case we get : indentation wrong, correct ourselves
    (define-key m "=" #'ally-mode-insert-and-indent)
    m)
  "Keymap used by Ally mode to implement electric keys.")

(defun ally-mode-insert-and-indent (key)
...
(defvar ally-mode-delayed-point nil
  "The point following the previous insertion if the insertion
was a delayed electric key.  Used to communicate between
`ally-mode-delayed-electric' and `ally-mode-delayed-electric-hook'.")
(make-variable-buffer-local 'ally-mode-delayed-point)

(defun ally-mode-delayed-electric (p)
...
point after the insertion of P, reindents the line containing P."
  ...
  (setq ally-mode-delayed-point (point)))

(defun ally-mode-delayed-electric-hook (b e l)
  "An after-change-function that implements `ally-mode-delayed-electric'."

  (when (and ally-mode-delayed-point
             (= ally-mode-delayed-point b))
    (save-excursion
      (save-match-data
        (goto-char ally-mode-delayed-point)
        (indent-according-to-mode))))
  (setq ally-mode-delayed-point nil))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Parser
;;

(defvar ally-mode-mark-cs-end 1
  "The point at which the comment/string cache ends.  The buffer
will be marked from the beginning up to this point (that is, up
to and including character (1- ally-mode-mark-cs-end)).")
(make-variable-buffer-local 'ally-mode-mark-cs-end)

(defvar ally-mode-mark-cs-state nil
  "The `parse-partial-sexp' state of the comment/string parser as
of the point `ally-mode-mark-cs-end'.")
(make-variable-buffer-local 'ally-mode-mark-cs-state)

(defvar ally-mode-mark-nesting-end 1
  "The point at which the nesting cache ends.  The buffer will be
marked from the beginning up to this point.")
(make-variable-buffer-local 'ally-mode-mark-nesting-end)

(defun ally-mode-mark-clear-cache (b e l)
  "An after-change-function that clears the comment/string and
nesting caches from the modified point on."

  (save-restriction
    (widen)
    (when (< b ally-mode-mark-cs-end)
      (remove-text-properties b (min ally-mode-mark-cs-end (point-max)) '(ally-mode-cs nil))
      (setq ally-mode-mark-cs-end b
            ally-mode-mark-cs-state nil))

    (when (< b ally-mode-mark-nesting-end)
      (remove-text-properties b (min ally-mode-mark-nesting-end (point-max)) '(ally-mode-nesting nil))
      (setq ally-mode-mark-nesting-end b))))

(defmacro ally-mode-parser (&rest body)
...
(defsubst ally-mode-cs (&optional pos)
...
comment or string."
  ...
  (if (= pos 1)
      nil
    (when (> pos ally-mode-mark-cs-end)
      (ally-mode-mark-cs pos))
    (get-text-property (- pos 1) 'ally-mode-cs)))

(defun ally-mode-mark-cs (end)
  "Mark comments and strings up to point END.  Don't call this
directly; use `ally-mode-cs'."

  (setq end (min end (point-max)))
  (ally-mode-parser
   (let* ((pos ally-mode-mark-cs-end)
          (state (or ally-mode-mark-cs-state (syntax-ppss pos))))
       ...
       (let ((start (nth 8 state)))
         (when start
           (setq state (parse-partial-sexp pos (point-max) nil nil state 'syntax-table)
                 pos (point))
           ;; Mark comment
           (put-text-property start (- pos 1) 'ally-mode-cs (cons start pos))
           (when nil
             (put-text-property start (- pos 1) 'face
                                `((:background "midnight blue")))))))
     ;; Update state
     (setq ally-mode-mark-cs-end   pos
           ally-mode-mark-cs-state state))))

(defsubst ally-mode-nesting (&optional pos)
...
encountered the close character."
  ...
  (if (= pos 1)
      '()
    (when (> pos ally-mode-mark-nesting-end)
      (ally-mode-mark-nesting pos))
    (get-text-property (- pos 1) 'ally-mode-nesting)))

(defun ally-mode-mark-nesting (pos)
...
`ally-mode-nesting'."
  ...
  (ally-mode-parser
   ;; Mark depth
   (goto-char ally-mode-mark-nesting-end)
   (let ((nesting (ally-mode-nesting))
         (last (point)))
     (while (< last pos)
       ...
       ;; nesting
       (put-text-property last (point) 'ally-mode-nesting nesting)
       (when nil
       ...
       ;; Update nesting
       (unless (eobp)
         (let ((ch (unless (ally-mode-cs) (char-after))))
           (forward-char 1)
           (case ch
             ((?\( ?\{ ?\[)
     ...
     ;; Update state
     (setq ally-mode-mark-nesting-end last))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Indentation
;;

(defvar ally-mode-non-terminating-keywords-regexp
  (let* ((kws ally-mode-keywords)
         (kws (remove "break" kws))
         (kws (remove "continue" kws))
         (kws (remove "fallthrough" kws))
         (kws (remove "return" kws)))
    (regexp-opt kws 'words))
  "Regular expression matching all Ally keywords that *do not*
implicitly terminate a statement.")

(defun ally-mode-semicolon-p ()
    ...
    (otherwise
     (save-excursion
       (when (/= (skip-chars-backward "[:word:]_") 0)
         (not (looking-at ally-mode-non-terminating-keywords-regexp)))))))

(defun ally-mode-indentation ()
        ...
        (let ((indent 0)
          ...
          (when (case (char-before)
                  ...
                  (t
                   ...
                   ;; semicolon?
                   (not (ally-mode-semicolon-p))))
            (incf indent tab-width))

          (max indent 0)))))))

(defun ally-mode-indent-line ()
  "Indent the current line according to `ally-mode-indentation'."
  (interactive)

  (let ((col (ally-mode-indentation)))
    (when col
...
;;;###autoload
(define-derived-mode ally-mode nil "Ally"
  "Major mode for editing Ally source text.

This provides basic syntax highlighting for keywords, built-ins,
functions, and some types.  It also provides indentation that is
\(almost) identical to allyfmt."

  ;; Font lock
  (set (make-local-variable 'font-lock-defaults)
       '(ally-mode-font-lock-keywords nil nil nil nil))

  ;; Remove stale text properties
  (save-restriction
    (widen)
    (remove-text-properties 1 (point-max)
                            '(ally-mode-cs nil ally-mode-nesting nil)))

  ;; Reset the syntax mark caches
  (setq ally-mode-mark-cs-end      1
        ally-mode-mark-cs-state    nil
        ally-mode-mark-nesting-end 1)
  (add-hook 'after-change-functions #'ally-mode-mark-clear-cache nil t)

  ;; Indentation
  (set (make-local-variable 'indent-line-function)
       #'ally-mode-indent-line)
  (add-hook 'after-change-functions #'ally-mode-delayed-electric-hook nil t)
...
;;;###autoload
(add-to-list 'auto-mode-alist (cons "\\.ally$" #'ally-mode))

(defun ally-mode-reload ()
  "Reload ally-mode.el and put the current buffer into Ally mode.
Useful for development work."

  (interactive)
  (unload-feature 'ally-mode)
  (require 'ally-mode)
  (ally-mode))

;;;###autoload
(defun allyfmt ()
 "Pipe the current buffer through the external tool `allyfmt`.
 ...
 (let ((srcbuf (current-buffer)))
   (with-temp-buffer
     (let ((outbuf (current-buffer))
           (errbuf (get-buffer-create "*Allyfmt Errors*"))
       ...
       (with-current-buffer srcbuf
         (save-restriction
           (let (deactivate-mark)
             (widen)
             (if (= 0 (shell-command-on-region (point-min) (point-max) "allyfmt"
                                               outbuf nil errbuf))
                 ;; allyfmt succeeded: replace the current buffer with outbuf,
               ...
               ;; allyfmt failed: display the errors
...
;;;###autoload
(defun allyfmt-before-save ()
 "Add this to .emacs to run allyfmt on the current buffer when saving:
 (add-hook 'before-save-hook #'allyfmt-before-save)"

 (interactive)
 (when (eq major-mode 'ally-mode) (allyfmt)))
